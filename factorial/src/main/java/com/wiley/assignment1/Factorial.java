...
public class Factorial {
	...
	public int isInputNumberValid() {
		...
		// Only take the user if its going to be an integer otherwise return -1
		String invalidMessage = "Invalid entry. Please enter an integer between 1 and 10, inclusive.";

		if (scanner.hasNextInt()) {

			this.num = scanner.nextInt();

			// Validate numeric user input is within the range of this method.
			if (this.num >= 1 && this.num <= 10) {
				return this.num;
			} else {
				this.printStream.print(invalidMessage);
				return -1;
			}
		}
